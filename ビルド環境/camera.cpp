...
void UpdateCamera(void)
{
	...
	if (g_Camera.state == CAMERA_STATE_NONE)
	{
		...
		if (GetKeyboardPress(DIK_E) || IsButtonTriggered(0, BUTTON_A))
		{
...
void SetCameraAT(XMFLOAT3 pos)
{
	...
	// プレイヤーの右前にする
	g_Camera.at.x = pos.x + sinf(g_Camera.rot.y + 0.35f) * 160.0f;
	g_Camera.at.z = pos.z + cosf(g_Camera.rot.y + 0.35f) * 160.0f;
	...
	// プレイヤーの右後
	g_Camera.pos.x = pos.x - sinf(g_Camera.rot.y - 0.30f) * 70.0f;
	g_Camera.pos.z = pos.z - cosf(g_Camera.rot.y - 0.30f) * 70.0f;
