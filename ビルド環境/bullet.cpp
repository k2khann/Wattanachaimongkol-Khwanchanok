...
void UpdateBullet(void)
{
	...
	for (int i = 0; i < MAX_BULLET; i++)
	{
		if (g_Bullet[i].use)
		{
			...
			// g_Bullet[i].pos.y -= sinf(g_Bullet[i].rot.y) * g_Bullet[i].spd;

			// 地雷バレット
			// g_Bullet[i].rot.y += 1.0f;


			// Particalの設定
			SetParticle(g_Bullet[i].pos, XMFLOAT3(0.5f, 0.5f, 0.5f), XMFLOAT3(0.0f, 0.0f, 0.0f), XMFLOAT4(0.0f, 0.8f, 1.0f, 0.75f), TYPE_SNOW, 10.0f, 10.0f, 60);
