...
#define	MAX_PARTICLE		(1020)		// パーティクル最大数
...
HRESULT InitParticle(void)
{
	...
	for (int nCntParticle = 0; nCntParticle < MAX_PARTICLE; nCntParticle++)
	{
		...
		g_aParticle[nCntParticle].type = TYPE_SNOW;
...
void UpdateParticle(void)
{
...

	{
		for (int nCntParticle = 0; nCntParticle < MAX_PARTICLE; nCntParticle++)
		{
			if (g_aParticle[nCntParticle].use)
			{
				...
				{// 着地した
					...
					g_aParticle[nCntParticle].move.y = -g_aParticle[nCntParticle].move.y * 0;

					g_aParticle[nCntParticle].move.x = 0;
					g_aParticle[nCntParticle].move.z = 0;
				}
				...
				{	// Particleどんどん消える
					if (g_aParticle[nCntParticle].nLife <= 60 && g_aParticle[nCntParticle].scale.x > 0.0f)
					{
						...
						g_aParticle[nCntParticle].scale.z -= 0.01f;
		...
		// パーティクル発生
		{
			...
			move.y = -0.5f; //rand() % 300 / 100.0f + g_fHeightBase;
			...
			pos.x = (float)(rand() % 1000 - 200);
			pos.y = 100;
			pos.z = (float)(rand() % 1000 - 200);

			// ビルボードの設定
			SetParticle(pos, XMFLOAT3(0.5f, 0.5f, 0.5f), move, XMFLOAT4(0.8f, 0.7f, 0.2f, 0.55f), TYPE_SNOW, 5.0f, 5.0f, nLife);
		}
	}
}
	...
	for (int nCntParticle = 0; nCntParticle < MAX_PARTICLE; nCntParticle++)
	{
		if (!g_aParticle[nCntParticle].use)
		{
			...
			if (g_aParticle[nCntParticle].nIdxShadow != -1)
			{
				...
				SetScaleShadow(g_aParticle[nCntParticle].nIdxShadow, XMFLOAT3(0.3f, 0.3f, 0.3f));
			}
#endif
